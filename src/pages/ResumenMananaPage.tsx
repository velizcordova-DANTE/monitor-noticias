...
export function ResumenMananaPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setToast({ message: `Resumen de la mañana creado con ${newsIds.length} noticias`, type: 'success' });
  ...
  return (
    <div className="page">
      ...
      <div className="page-header">
        <div>
          <h1>Resumen Express · Mañana</h1>
          <p>Escané las fuentes y seleccioná rápido hasta {MAX_NEWS} noticias para el monitoreo de la mañana</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="form" style={{ maxWidth: 700 }}>
            ...
            {scanResults.length === 0 && !scanning && !scanError && (
              <div style={{ textAlign: 'center', padding: '16px 0' }}>
                <p style={{ marginBottom: 16, color: 'var(--gray-600)' }}>
                  Escaneá las noticias más recientes de todas las fuentes bolivianas.
                </p>
            ...
            {scanResults.length > 0 && !scanning && (
              <div>
                <p style={{ marginBottom: 12, color: 'var(--gray-600)' }}>
                  {scanResults.length} noticias encontradas. Seleccioná las que querés incluir.
                </p>
                ...
                <div style={{ marginTop: 8, display: 'flex', gap: 8 }}>
                  ...
                  {scanResults.length > 0 && <><button type="button" className="btn btn-sm" style={{ background: '#e8f5e9', borderColor: '#a5d6a7' }} onClick={() => {
                    ...
                    a.href = url;
                    a.download = `resumen-manana-${form.date || new Date().toISOString().slice(0, 10)}.html`;
          ...
          <div className="form-actions">
            ...
            <button type="submit" className="btn btn-primary" disabled={saving || selectedArticles.size === 0}>
              {saving ? 'Creando...' : `Crear Resumen Mañana (${selectedArticles.size} noticias)`}
            </button>
          </div>
        </form>
      )}
