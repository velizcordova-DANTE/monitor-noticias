...
export function ExpressSummaryPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setToast({ message: `Resumen express creado con ${newsIds.length} noticias`, type: 'success' });
  ...
  return (
    <div className="page">
      ...
      <div className="page-header">
        <div>
          <h1>Resumen Express</h1>
          <p>Escané las fuentes y seleccioná rápido hasta {MAX_NEWS} noticias para el monitoreo de hoy</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="form" style={{ maxWidth: 700 }}>
          ...
          <div className="card" style={{ padding: 20, marginBottom: 20 }}>
            ...
            {scanResults.length === 0 && !scanning && !scanError && (
              <div style={{ textAlign: 'center', padding: '16px 0' }}>
                <p style={{ marginBottom: 16, color: 'var(--gray-600)' }}>
                  Escaneá las {MAX_NEWS} noticias más recientes de todas las fuentes bolivianas.
                </p>
            ...
            {scanResults.length > 0 && !scanning && (
              <div>
                ...
                <button type="button" className="btn btn-sm" style={{ marginTop: 8 }} onClick={runScan}>
                  Re-escanear
                </button>
              </div>
            )}
          ...
          <div className="form-actions">
            ...
            <button type="submit" className="btn btn-primary" disabled={saving || selectedArticles.size === 0}>
              {saving ? 'Creando...' : `Crear Resumen Express (${selectedArticles.size} noticias)`}
            </button>
          </div>
        </form>
      )}
